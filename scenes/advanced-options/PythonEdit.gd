...
func from_string_matrix(string):
	var m_row_strings = string.split(",", true)
	if len(m_row_strings) == 4:
		...
		return Contraction.nearest_allowed_matrix(matrix) # transform into allowed one
...
func load_text(ifs):
	...
	for i in len(ifs.systems):
		if i < len(contraction_substring_array)-1:
			...
			var new_substring = ""
			# change translation
			var vars = read_vars_for_contraction(substring)
			...
			# change matrix
			vars = read_vars_for_contraction(substring)
			new_substring = ""
...
func read_vars():
	...
	if position_of_string_delay >= 0:
		...
		for c in last_possible_one - delay_input_pos[0]:
			if text[delay_input_pos[0] + c] not in MATH_SYMBOLS:
				delay_input_pos[1] = delay_input_pos[0] + c
				break

func read_vars_for_contraction(substring):
	var pos_dict = {}
...
func read_text():
	...
	for substring in substrings:
		...
		var matrix = from_string_matrix(get_substring(substring, vars["matrix"][0], vars["matrix"][1]))
		if matrix:
...
func round_numbers():
	...
	if len(contraction_substring_array) > 0: # "preliminaries" or something
		substring_array.append(contraction_substring_array[0])
	# add substrings for each ifs
	for i in len(contraction_substring_array)-1:
		var substring = contraction_substring_array[i+1]
		var new_substring = ""
		# round translation
		var vars = read_vars_for_contraction(substring)
		if vars["translation"][0] >= 0 and vars["translation"][1] >= 0:
			...
			if len(t_string_array) == 2:
				...
				# round vars
				new_substring += get_substring(substring, 0, vars["translation"][0]) # before translation values
				new_substring += str(snapped(translation.x, DIGITS)) + ", " + str(snapped(translation.y, DIGITS)) # t values
				new_substring += get_substring(substring, vars["translation"][1], substring.length()) # after t values
				substring = new_substring
		# change matrix
		vars = read_vars_for_contraction(substring)
		...
		if matrix:
			new_substring += get_substring(substring, 0, vars["matrix"][0]) # before m values
			new_substring += "[" + str(snapped(matrix[0], DIGITS)) + ", " + str(snapped(matrix[1], DIGITS)) + "], " # first row
			new_substring += "[" + str(snapped(matrix[2], DIGITS)) + ", " + str(snapped(matrix[3], DIGITS)) + "]"
			new_substring += get_substring(substring, vars["matrix"][1], substring.length()) # after m values
			substring = new_substring
		# save substring
		substring_array.append(substring)
